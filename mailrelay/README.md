...
| `content` | 二选一 | 正文（与 `html` 至少有一个非空）。普通文字按纯文本发；**内容里含 HTML 标签（`<table>`/`<div>`/`<br>` 等）会自动识别为 HTML 发送**，并附纯文本备选（像 `a < b` 这种不会误判） |
| `html` | 二选一 | 显式 HTML 正文；给了它就按 HTML 发，此时 `content` 留作纯文本备选，不给则由 HTML 自动粗转 |
