...
async function saveViewCycle() {
  await api('/api/auth/todo-view-list', { method: 'PUT', body: { list: viewCycle } });
}
// 排序/删除（事件委托）
document.getElementById('viewCycleList').addEventListener('click', async function(e){
  ...
  if (b.hasAttribute('data-vup')) {
    ...
    if (i < viewCycle.length - 1) { var t1 = viewCycle[i + 1]; viewCycle[i + 1] = viewCycle[i]; viewCycle[i] = t1; }
  } else {
    viewCycle.splice(i, 1);
  }
  renderViewCycle();
...
document.getElementById('resetViewBtn').addEventListener('click', async function(){
  viewCycle = [];
  renderViewCycle();
  try {
...
if (mottoInput) {
  ...
  document.getElementById('mottoSave').addEventListener('click', async function(){
    ...
    try {
      ...
      if (ov && text && ov.classList.contains('is-' + mottoStyle)) {
        ...
        q.textContent = text;
        q.setAttribute('data-len', Array.from(text).length <= 14 ? 's' : (Array.from(text).length <= 30 ? 'm' : 'l'));
      }
      showMsg(msg, text ? '勉励卡已保存' : '已清空，明天起不再展示卡片', true);
    } catch(err) { showMsg(msg, err.message, false); }
  });
  // 预览：先保存再刷新，用 sessionStorage 通知新页面打开卡片（覆盖今天已读、DOM 未直出等情况）
  ...
  // 刷新后若带预览标记，自动打开服务端按最新数据直出的卡片
  if (sessionStorage.getItem('mottoPreview') === '1') {
    sessionStorage.removeItem('mottoPreview');
    setTimeout(function(){ if (window.__mottoPreview) window.__mottoPreview(); }, 350);
  }
}
...
function renderSummary(report, goal, year) {
  ...
  var changes = validChanges(series);
  var latestChange = report.netWorthChange != null && isFinite(report.netWorthChange)
    ? report.netWorthChange
    : (changes.length ? changes[changes.length - 1] : null);
  var avg6 = avgNum(validChanges(series, 6));
  var avg12 = avgNum(validChanges(series, 12));
  var total12 = sumNum(validChanges(series, 12));
  var diff6 = latestChange != null && avg6 != null ? latestChange - avg6 : null;
  var lowIndex = -1, lowValue = null;
  var start = Math.max(0, series.length - 12);
  for (var i = start; i < series.length; i++) {
    var v = series[i];
    if (v == null || !isFinite(v)) continue;
    if (lowValue === null || v < lowValue) { lowValue = v; lowIndex = i; }
  }
...
function assetCssVar(name, fallback) {
  var v = getComputedStyle(document.documentElement).getPropertyValue(name).trim();
  return v || fallback;
}
function drawCharts(report) {
  var brandColor = assetCssVar('--brand-strong', '#7C3AED');
  var okColor = assetCssVar('--ok', '#34b34a');
  var dangerColor = assetCssVar('--danger', '#cf1322');
  var avgColor = '#f97316';
...
function todoRootDue(root) {
  ...
  (function walk(n, inherited){
    ...
    // child_due 空壳(任意层级)是分组容器, 不用继承来的日期冒充到期叶子(与后端 rootDueOf 一致)
    if (!n.child_due && own && (!min || own < min)) min = own;
  })(root, null);
  return min;
}
...
function todoApplyViewCycle(list, getRowsFn, onDrawTree) {
  ...
  if (_todoViewCycle.indexOf(_todoView) < 0) {
    ...
    _todoDetailRootId = null;
    try { localStorage.setItem('todoView', _todoView); } catch(e){}
    if (typeof getRowsFn === 'function') applyTodoView(getRowsFn, onDrawTree);
  }
}
...
function todoBuildTree(rows) {
  ...
  // 给每个节点挂顶层主任务指针 _root 与直接父指针 _parent(根为 null),
  // 供渲染徽章/表单门控(逐级 child_due 只看直接父)/内联添加表单读取
  // 注意: 必须用函数声明(或具名函数表达式后立即赋值给变量), 不能写 (function tag(){...});
  // 命名函数表达式的名字只在函数自身作用域可见, 外部 forEach 里调 tag 会 ReferenceError
  function tag(n, root, parent) {
    n._root = root;
...
function renderTodoAccordion(container, trees, opts) {
  ...
  function buildOps(node, depth, rowEl, onAddClick) {
    ...
    var dragHandle = null;
    var isRealRoot = depth === 0 && !isDetail;
    if (opts.onReorder && !isRealRoot) {
      dragHandle = document.createElement('button');
      dragHandle.type = 'button'; dragHandle.className = 'todo-op todo-drag'; dragHandle.title = '拖动排序';
      dragHandle.innerHTML = ICONS.drag;
      opsEl.appendChild(dragHandle);
    }
    if (opts.onAddChildSubmit) {
    ...
    if (opts.onShare && depth === 0 && node.parent_id == null) {
      var b3 = document.createElement('button');
      b3.type = 'button'; b3.className = 'todo-op'; b3.title = '协作链接'; b3.innerHTML = ICONS.share;
      b3.addEventListener('click', function(e){
        e.stopPropagation(); opts.onShare(node);
      });
      opsEl.appendChild(b3);
    }
    if (opts.onDel) {
      var b4 = document.createElement('button');
      b4.type = 'button'; b4.className = 'todo-op danger'; b4.title = '删除'; b4.innerHTML = ICONS.trash;
      b4.addEventListener('click', function(e){
        e.stopPropagation(); opts.onDel(node);
      });
      opsEl.appendChild(b4);
    }
    if (opsEl.childNodes.length) {
      var bMore = document.createElement('button');
      bMore.type = 'button'; bMore.className = 'todo-op todo-more'; bMore.title = '更多操作'; bMore.innerHTML = ICONS.more;
      bMore.addEventListener('click', function(e){
        e.stopPropagation(); todoOpMenuToggle(rowEl, opsEl);
      });
      opsEl.appendChild(bMore);
    }
    return opsEl.childNodes.length ? opsEl : null;
  }

  // 叶子行：勾选圆 + 标题 + 悬停操作组 + 逾期日期
  function leafNode(node, depth, effDue) {
    var wrap = document.createElement('div');
  ...
  trees.forEach(function(t){
    var el = walk(t, startDepth, opts.forcedRootDue != null ? opts.forcedRootDue : t.due_date);
...
// options.isDetail: 详情页内行 ＋(显示"添加到"面包屑); options.onAddForRoot: ✕ 改为主任务时展开主任务卡片
...
// 逐级门控(开关只管一级): 子任务能否自设日期只看【直接父】是否勾选 child_due
// fopts:
...
function todoFormRead() {
  ...
  // child_due 勾选框存在(主任务/自由子任务)才输出该键; 锁定跟随态与协作页根任务不渲染, 后端即不切换模式
...
// 逐级门控: 统计 node 子树中「有有效截止日(自身优先否则继承祖先)且未完成」的叶子数
// (只数后代不含 node 自身; 已完成分支整支跳过; child_due 空壳不计), 与后端 countStats 同口径。
function todoPendingDatedLeaves(node) {
  var n = 0;
  function walk(x, inheritedDue) {
    if (x.done) return;
    var own = x.due_date || inheritedDue;
    if (x.children && x.children.length) {
      x.children.forEach(function(c){ walk(c, own); });
      return;
    }
    if (!x.child_due && own) n++;
  }
  (node.children || []).forEach(function(c){ walk(c, node.due_date || null); });
  return n;
}
// 子树内是否存在逐级开关(node 自身或任一后代勾选 child_due):
// 只有含独立日期子任务的自由子树, 勾选父任务完成前才需要二次确认
function todoSubtreeHasChildDue(node) {
  if (node.child_due) return true;
  return (node.children || []).some(todoSubtreeHasChildDue);
}
// 勾选未完成→完成前的二次提醒: node 自身或后代勾了 child_due(子任务有独立截止时间),
// 且其下仍有带有效截止时间的未完成叶子时弹窗; 传统整枝(日期全继承)/无未完成叶子直接放行。
// 返回 Promise: true=继续提交, false=用户取消
function todoConfirmDoneIfPending(node, done) {
  if (!done) return Promise.resolve(true);
  if (!todoSubtreeHasChildDue(node)) return Promise.resolve(true);
  var pending = todoPendingDatedLeaves(node);
  if (!pending) return Promise.resolve(true);
  return new Promise(function(resolve){
    var msg = '该任务下还有 ' + pending + ' 个带截止时间的子任务未完成。\\n仍要标记为已完成吗？\\n确认后这些子任务将随该任务一并视为完成。';
    openModal('确认完成任务',
...
function todoCelebrationCount(trees, datedOnly) {
  ...
  function walk(node, inheritedDue) {
    ...
    if (node.child_due) return; // 空壳分组容器不计(与 countStats 叶子口径一致)
...
function openTodoEdit(node) {
  ...
  bindClickBusy(document.getElementById('tfSave'), async function(){
    ...
    // 自由子任务(直接父勾选)且未勾自身开关时, 截止日期必填(与快捷添加框同口径)
    if (isChild && fopts.childDueMode && body.child_due !== 1 && !body.due_date) { alertModal('请选择截止日期（默认今天），或勾选"子任务各自设置截止日期"', {ok:false}); return; }
...
function openPublicEdit(node) {
  ...
  // 逐级门控: 子任务看【直接父】; 协作页不允许切换任何开关(root 锁定, 子任务表单不输出勾选框)
  var _byId = {}; _rows.forEach(function(r){ _byId[r.id] = r; });
  var fopts = isChild
...
function openAddForm(parentId, title) {
  // 单清单页恒为子任务: 按【直接父】(缺省挂根任务) child_due 决定日期/重复控件(新建即为叶子)
