...
function settingsPage(user) {
  const body = renderTopbar(user, 'settings') + `<div class="container">
    <div class="card m-only">
      <h2>功能入口</h2>
      <div class="m-entry">
        <a href="/dashboard"><svg viewBox="0 0 24 24" width="17" height="17" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7" rx="1.5"/><rect x="14" y="3" width="7" height="5" rx="1.5"/><rect x="14" y="12" width="7" height="9" rx="1.5"/><rect x="3" y="14" width="7" height="7" rx="1.5"/></svg>仪表盘</a>
  ...
  const body = renderTopbar(user, 'todo') + `<div class="container">
    ...
    <div class="card">
      <div class="todo-chart-head">
        <h2 style="margin:0;display:flex;align-items:center;gap:8px;flex:1;min-width:0;">任务趋势 <button type="button" class="btn sm gray todo-analyze-btn" id="todoAnalyzeBtn">📊 分析</button><button type="button" class="btn sm todo-chart-refresh" title="刷新趋势图" style="margin-left:auto;"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:block;"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg></button></h2>
  ...
  <div id="todoFullscreen" class="todo-fullscreen">
    ...
    <div class="todo-fs-main">
      <div class="todo-fs-top">
        ...
        <button class="fs-viewswitch" id="viewSwitchBtn" aria-label="切换视图" title="切换视图">${ICO_TREE}</button>
        <span class="fs-viewcur-text" id="viewCurrentLbl">${ICO_TREE}<span class="fs-viewcur-text-name">完整树</span></span>
